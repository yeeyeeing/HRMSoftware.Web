...
import { EmployeeAllowanceRow, EmployeeAllowanceService, EmployeeProfileService } from '../../ServerTypes/EmployeeProfile';
import { EisSubjectionService, EpfSubjectionService, HrdfSubjectionService, NoPaidLeaveRow, NoPaidLeaveService, PayrollEarningsRow, PayrollForm, PayrollRow, PayrollService, PcbSubjectionService, SocsoSubjectionService } from '../../ServerTypes/PayrollSettings';
...
export class PayrollDialog extends EntityDialog<PayrollRow, any> {
    ...
    public dialogOpen(asPanel?: boolean): void {
        ...
        CompanySettingsService.List({
            ...
            var LastMonth = new Date(dateFormat)
            LastMonth.setMonth(LastMonth.getMonth() - 1);
            LastMonth.setDate(LastMonth.getDate() - 1);
            var LastMonthObjYear = LastMonth.getFullYear().toString()
            var LastMonthObjMonth = LastMonth.getMonth().toString()
                    ...
                    for (var index in listOfDicts) {
                        if (listOfDicts[index].id == $(EmployeeRowIdElement).val()) {
                            ...
                            EmployeeAllowanceService.List({
                                ...
                                var result: any[] = []
                                for (var index in response.Entities) {
                                    var EffectiveFromValue = new Date(response.Entities[index].EffectiveFrom)
                                    if (self.form.PayPeriodStart.valueAsDate >= EffectiveFromValue) {
                                        ...
                                        buffer.SubjectionToEis = (response.Entities[index].SubjectionEis ? 1 : 0)
                                        buffer.SubjectionToEpf = (response.Entities[index].SubjectionEpf ? 1 : 0)
                                        buffer.SubjectionToHrdf = (response.Entities[index].SubjectionHrdf ? 1 : 0)
                                        buffer.SubjectionToPcb = (response.Entities[index].SubjectionPcb ? 1 : 0)
                                        buffer.SubjectionToSocso = (response.Entities[index].SubjectionSocso ? 1 : 0)
                                        result.push(buffer)
                                    }
                                }
                                self.form.PayrollEarnings.value = result
                                ...
                                OTApplicationService.List({
                                    ...
                                    for (var index in response.Entities) {
                                        if (response.Entities[index].Paid == 0 && response.Entities[index].Status == 1)
                                    ...
                                    NoPaidLeaveService.List({
                                        ...
                                        MoneyClaimApplicationService.List({
                                            Criteria: [[MoneyClaimApplicationRow.Fields.EmployeeRowId], '=', $(EmployeeRowIdElement).val()]
                                        }, response => {
                                            for (var index in response.Entities) {
                                                if (response.Entities[index].Paid == 0 && response.Entities[index].Status == 1) {
