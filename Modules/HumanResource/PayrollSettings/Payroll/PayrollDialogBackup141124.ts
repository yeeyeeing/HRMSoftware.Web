...
import { EisSubjectionService, EpfSubjectionService, HrdfSubjectionService, NoPaidLeaveRow, NoPaidLeaveService, PayrollDeductionsRow, PayrollEarningsRow, PayrollForm, PayrollRow, PayrollService, PcbSubjectionService, SocsoSubjectionService } from '../../ServerTypes/PayrollSettings';
...
export class PayrollDialog extends EntityDialog<PayrollRow, any> {
    ...
    public GeneratePayrollTable(): void {
        var self = this
        var TableNode = document.getElementById('PayrollTable')
        ...
        if (isEmptyOrNull($(EmployeeRowIdElement).val()) || isEmptyOrNull($(PayYearElement).val()) || isEmptyOrNull($(PayMonthElement).val()))
            return
        ...
        if (isEmptyOrNull($(EmployeeRowIdElement).val())) {
            ...
            function doRangesOverlap(start1, end1, start2, end2) {
                ...
                const dateStart1 = new Date(start1);
                const dateEnd1 = new Date(end1);
                const dateStart2 = new Date(start2);
                const dateEnd2 = new Date(end2);

                // Check if the ranges overlap
                return dateStart1 < dateEnd2 && dateEnd1 > dateStart2;
            }
                ...
                NoPaidLeaveService.List({
                    ...
                    EmployeeEarlyLeavingService.List({
                        ...
                        LeaveApplicationService.List({
                            ...
                            // Initialize all leave variables to false

                            // Iterate over the response entities
                            for (var res in response.Entities) {
                                var leaveReasonId = response.Entities[res].LeaveReasonId;

                                // Set the corresponding leave flag to true based on the LeaveReasonId
                                        ...
                                        if (listOfDicts[index].id == $(EmployeeRowIdElement).val()) {
                                            ...
                                            EmployeeAllowanceService.List({
                                                ...
                                                OTApplicationService.List({
                                                    //Criteria: [[OTApplicationRow.Fields.EmployeeRowId], '=', $(EmployeeRowIdElement).val()]
                                                    Criteria: Criteria.and(criteria, [[OTApplicationRow.Fields.EmployeeRowId], '=', $(EmployeeRowIdElement).val()]
                                                        , [[OTApplicationRow.Fields.Status], '=', '1'], [[OTApplicationRow.Fields.Paid], '=', '0'])
                                                }, response => {
                                                    for (var index in response.Entities)
                                                        console.log(response.Entities[index])
                                                    ...
                                                    NoPaidLeaveService.List({
                                                        ...
                                                        FixedDeductionService.List({
                                                            ...
                                                            for (var index in response.Entities) {
                                                                ...
                                                                DeductionsString.push(response.Entities[index].Description + " : " + response.Entities[index].Amount + " : readonly")
                                                                    ...
                                                                    onSuccess: (response) => {
                                                                        ...
                                                                        if (response.Entities[0].OtRate != -1 && response.Entities[0].OtMinute != -1 && response.Entities[0].TotalOtPay != -1) {
                                                                            ...
                                                                            for (var index in aggregatedData) {
                                                                                ...
                                                                                buffer.SubjectionToSocso = (Socso == 1 ? true : false)
                                                                                buffer.Amount = BasicPay
                                                                                buffer.Description = 'Overtime Payments: RM' + parseFloat(aggregatedData[index].OtRate).toFixed(2).toString() + 'x' + OtHour.toString() + 'hour = ' + aggregatedData[index].TotalOtPay.toString()
                                                                                Earnings.push(buffer)
    ...
    public ValueChangeCallback(): void {
        ...
        for (var i = 0; i < TableBodyRows.length; i++) {
            ...
            if (EarningsAmount.val() != '' && EarningsDescription.text() != '') {
                this.EarningsString.push((EarningsDescription.text() + (EarningsDescription.text().startsWith("Overtime Payments:") ? ' = ' : ' : ') + BufferEarningAmount.toFixed(2)).toString() + (EarningReadOnly ? " : readonly" : '').toString())
    ...
    public PartialValueChangeCallback(): void {
        ...
        for (var i = 0; i < TableBodyRows.length; i++) {
            ...
            if (EarningsAmount.val() != '' && EarningsDescription.text() != '') {
                this.EarningsString.push((EarningsDescription.text() + (EarningsDescription.text().startsWith("Overtime Payments:") ? ' = ' : ' : ') + BufferEarningAmount.toFixed(2)).toString() + (EarningReadOnly ? " : readonly" : '').toString())
                console.log(EarningsDescription.text())
