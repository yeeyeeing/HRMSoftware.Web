...
return false; // Target node not found
    ...
    function listChildrenId(data: { id: string; parentId: string | null }
...
function DisableOrgChartMovement()
{ //callback for node on orgchart
                                ...
                                if (!isEmptyOrNull(StartPointData))
        {
            ...
            var StartPointParentId = StartPointJson.parentId
                ...
                'EmployeeRowId': null,
                                        ...
                                        'title': StartPointTitle, 'className': `${ StartPointId}
                Class`,
            ...
            if (StartPointHierarchy == EmployeeEnum)
            {
                bufferNode.EmployeeRowId = EmployeeRowId
                                        bufferNode.className = `EMPLOYEE${ EmployeeRowId}
                Class`
                                    }
            if (StartPointHierarchy == EmployeeEnum)
            {
                                ...
                                if (!isEmptyOrNull(TargetedFinalOrgChartBuffer))
            {
                ...
                */
                                    ...
                                    if (!isEmptyOrNull(childrenToAppend))
                {
                    ...
                    {
                        childrenToAppend[i].id = `${ parentOfElementToRemove.id}${
                            EncodeString(stripClassSuffix(childrenToAppend[i].className), 0x88, 0x99)
                                        }`
            ...
            if (node.children)
{
    ...
    var id = EncodeString(`EMPLOYEE${ ListOfEmployeeData[i]["id"]}`, 0x88, 0x99)
        ...
        'hierarchyId': ListOfEmployeeData[i]["id"],
                ...
                'id': id, 'name': str,
                'title': str, 'className': `EMPLOYEE${ ListOfEmployeeData[i].id}
        Class`
            };
    var PrevDestination;
    let counter = 0;
    ...
    for (const key of Object.keys(CardType)) {
        if (isNaN(Number(key)))
        {
            if (isEmptyOrNull(ListOfEmployeeData[i][`${ capitalizeFirstLetter(key)}
            Id`]))
                        continue
                    var DestinationNode = `${ key}${ ListOfEmployeeData[i][capitalizeFirstLetter(key) + 'Id']}
            Class`
    ...
    function GenerateUl(node, type)
{
    ...
    if (node.children && node.children.length > 0 && node.hierarchyLevel !== CardType.DIRECTOR)
    {
        ...
        toggleSpan.classList.add('toggle', checkbox.className);
        ...
        toggleSpan.addEventListener('click', () => {
            toggleChildNodes(li, toggleSpan, checkbox.checked);
        });
...
childUls.forEach(childUl => {
    childUl.style.display = isExpanding ? 'block' : 'none';
});

// Update the toggle button symbol for ALL child nodes
const allChildToggles = parentLi.querySelectorAll('.toggle');
allChildToggles.forEach(toggle => {
    (toggle as HTMLElement).textContent = isExpanding ? '-' : '+';
});
...
function GenerateOrgChart()
{
    ...
    {
        // Wait for OrgChart to render
        setTimeout(() => {
            const orgChartContainer = document.querySelector('#orgChartContainer'); // Adjust based on your chart's container ID
            const parentNode = document.querySelector('.orgchart .DIRECTOR'); // Select the top-most parent node

            if (parentNode && orgChartContainer)
            {
                // Scroll the container to the parent node
                orgChartContainer.scrollTo({
                top: parentNode.offsetTop - orgChartContainer.offsetHeight / 2,
                                left: parentNode.offsetLeft - orgChartContainer.offsetWidth / 2,
