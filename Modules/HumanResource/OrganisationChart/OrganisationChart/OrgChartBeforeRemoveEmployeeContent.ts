...
export default function pageInit() {
    ...
    function extractEmployeeInOrgChart(node) {
        ...
        if (node.nodeContentPro !== undefined && !isEmptyOrNull(node.nodeContentPro))
            results.push(node.nodeContentPro);
    ...
    OrganisationChartService.List({
            ...
            DivisionService.List({}, response => {
                ...
                SectionService.List({}, response => {
                    ...
                    EmployeeProfileService.List({}, response => {
                        EmployeeInOrgChart = extractEmployeeInOrgChart(datascource2)
                        ...
                        for (var index in response.Entities) {
                            ...
                            if (EmployeeInOrgChart.includes(ElementId))
                                continue
                            //var CardText = response.Entities[index].EmployeeName
                            //var EmployeeCard = GenerateCard(ElementId, CardText, CardType.EMPLOYEE)
                            var EmployeeCard = CreateEmployeeCard(ElementId, response.Entities[index].EmployeeName, response.Entities[index].EmployeeID,
                                response.Entities[index].OccupationID, response.Entities[index].DepartmentID,
                                response.Entities[index].DivisionID, response.Entities[index].SectionID,
                                response.Entities[index].EmployeeImg)
        ...
        // EmployeeTableButton.addEventListener('click', function () { ArrangePanel("EMPLOYEE"); console.log('haha')  }, false);
                            ...
                            ElementArray[j].addEventListener("drop", function (event) {
                                ...
                                if (!isEmptyOrNull(data)) {
                                    ...
                                    if (hierarchy != EmployeeEnum) {
                                        ...
                                        if (!isEmptyOrNull(EventTargetJson.nodeContentPro)) {
                                            //replace a employee card at the panel
                                            var employeeRowid = EventTargetJson.nodeContentPro
                                            let ListCounter = 0, found = 0
                                            for (ListCounter = 0; ListCounter < ListOfEmployeeData.length; ListCounter++) {
                                                if (ListOfEmployeeData[ListCounter].id == employeeRowid) {
                                                    found = 1
                                                    break
                                                }
                                            }
                                            var EmployeeCard = CreateEmployeeCard(ListOfEmployeeData[ListCounter].id, ListOfEmployeeData[ListCounter].EmployeeName,
                                                ListOfEmployeeData[ListCounter].EmployeeId,
                                                ListOfEmployeeData[ListCounter].OccupationId, ListOfEmployeeData[ListCounter].DepartmentId,
                                                ListOfEmployeeData[ListCounter].DivisionId, ListOfEmployeeData[ListCounter].SectionId,
                                                ListOfEmployeeData[ListCounter].ImgPath)
                                            var EmployeeEnumName = getEnumNameFromValue(EmployeeEnum)
                                            var TargetTable = document.body.querySelector("#" + capitalizeFirstLetter(EmployeeEnumName) + "Table") as HTMLElement
                                            console.log(TargetTable)
                                            if (TargetTable.children.length == 0) {
                                                var bufferDif = document.createElement('div')
                                                bufferDif.appendChild(EmployeeCard)
                                                TargetTable.appendChild(bufferDif)

                                            }
                                            else
                                                TargetTable.children[0].appendChild(EmployeeCard)
                                            var EnumName = getEnumNameFromValue(EventTargetJson.nodeTitlePro)
                                            ArrangePanel(EnumName)
                                        }
                                        console.log(datascource2)
                                    }
    ...
    function CreateEmployeeCard(EmployeeRowId, EmployeeName, EmployeeId,
        OccupationId, DepartmentId, DivisionId, SectionId, EmployeeImg) {
        ...
        for (var LookupIndex in DepartmentLookup.items) {
            if (DepartmentLookup.items[LookupIndex].Id == DepartmentId) {
                EmployeeDepartment = DepartmentLookup.items[LookupIndex].Name.replace(/\s/g, "")
                break
            }
        }
        var EmployeeDivision = null
        for (var LookupIndex in DivisionLookup.items) {
            if (DivisionLookup.items[LookupIndex].Id == DivisionId) {
                EmployeeDivision = DivisionLookup.items[LookupIndex].Name.replace(/\s/g, "")
                break
            }
        }
        var EmployeeSection = null
        for (var LookupIndex in SectionLookup.items) {
            if (SectionLookup.items[LookupIndex].Id == SectionId) {
                EmployeeSection = SectionLookup.items[LookupIndex].Name.replace(/\s/g, "")
                break
            }
        }
        ...
        nameSpan.style.display = 'block'; // Make sure this is displayed as a block-level element

        var idSpan = document.createElement('span');
        idSpan.textContent = EmployeeId;
        idSpan.style.display = 'block'; // Make sure this is displayed as a block-level element



        TitleContent.appendChild(nameSpan)
        TitleContent.appendChild(idSpan)

        var DivContent = document.createElement('div')

        var occupationSpan = document.createElement('span');
        occupationSpan.textContent = EmployeeOccupation;
        occupationSpan.style.display = 'block'; // Make sure this is displayed as a block-level element

        var departmentSpan = document.createElement('span');
        departmentSpan.textContent = "Department: " + EmployeeDepartment;
        departmentSpan.style.display = 'block'; // Make sure this is displayed as a block-level element
        var divisionSpan = document.createElement('span');
        divisionSpan.textContent = "Division: " + EmployeeDivision;
        divisionSpan.style.display = 'block'; // Make sure this is displayed as a block-level element
        var sectionSpan = document.createElement('span');
        sectionSpan.textContent = "Section: " + EmployeeSection;
        sectionSpan.style.display = 'block'; // Make sure this is displayed as a block-level element



        //DivContent.appendChild(occupationSpan);
        DivContent.appendChild(departmentSpan);
        DivContent.appendChild(divisionSpan);
        DivContent.appendChild(sectionSpan);

        //DivContent.textContent = EmployeeOccupation + '<br>' + EmployeeDepartment
        DivContent.className = 'CardContent'

        var DivContent2 = document.createElement('div')
        DivContent2.textContent = EmployeeDepartment
        DivContent2.className = 'CardContent'
        EmployeeCol_1.appendChild(fragment)
        EmployeeCol_2.appendChild(TitleContent)
        EmployeeCol_2.appendChild(DivContent)
        //EmployeeCol_2.appendChild(DivContent2)
