...
export class ViewShiftHistoryDialog extends EntityDialog<ViewShiftHistoryRow, any> {
    ...
    constructor(EmployeeID: string, EmployeeRowID: number) {
        ...
        this.form.EmployeeId.value = this.EmployeeID;
    ...
    public dialogOpen(asPanel?: boolean): void {
        ...
        /*
        function generateChartData(data, ListOfGroupData, ListOfShiftData, ListOfPublicHolidayData, ListOfTakenLeave)
        {
            ...
            for (let day = 1; day <= monthLength; day++)
            {
                ...
                else if (Group != null)
                {
                    ...
                    for (var index in ListOfGroupData)
                    {
                        ...
                        {
                            ShiftString = ListOfGroupData[index].GroupName
                ...
                for (var index in ListOfTakenLeave)
                {
                  
                    var StartingRange = new Date(ListOfTakenLeave[index].StartDate)
                    var EndingRange = new Date(ListOfTakenLeave[index].EndDate)
                    var currentValue = ListOfTakenLeave[index].LeaveReason
                    var key
                                        ...
                                        if (Data[buffer].x == this.x && Data[buffer].y == this.y) {
                                            ...
                                            */
                                            for (var each_res in CopiedResEntity) {
                                                ...
                                                if (parseInt(CopiedResEntity[each_res]["Id"]) == 0)
                                                    no_records = true
                                            ...
                                            */
                                            ...
                                            for (var each_res in CopiedResEntity) {
                                                ...
                                                if (no_records == false) {
                                                    ...
                                                    var endDateString = CopiedResEntity[each_res].ShiftEndDate.substring(0, 10)
                                                    DateStringList.push(startDateString)
                                                }


                                                /*
                                                if (isPublicHoliday == 0)
                                                {
                                                    if (isDateBetween(DatetimeDate, startDate, endDate))
                                                    {
                                                        FormID = CopiedResEntity[each_res].Id
                                                        var dlg2 = new SetEmployeeShiftDialog(EmployeeRowID, formattedDate, 1, endDateString);//open the shift 
                                                        dlg2.loadByIdAndOpenDialog(FormID)
                                                        dlg2.element.on("dialogclose", function () {
                                                            serviceCall<ListResponse<any>>({
                                                                service: ViewShiftHistoryService.baseUrl + '/RetriveShiftHistory',
                                                                method: "GET",
                                                                data: {
